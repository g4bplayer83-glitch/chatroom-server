...
const icons = {
  ...
  screen: "M2 4h20v14H2Z M8 22h8 M12 18v4",
  pin: "m8 3 8 0 -1 6 4 4H5l4-4Z M12 13v9",
...
function avatarHTML(name, url) {
  const src = safePath(state.profiles?.[keyName(name)]?.avatar || url);
  return `<span class="avatar" data-avatar-user="${esc(name)}">${
    src
...
function applyPreferences() {
  window.DocSpaceFeatures?.applyTheme();
  document.body.classList.toggle("light", ["light", "pink-light"].includes(preferences.theme));
...
window.DocSpace = {avatarHTML, socket, state, toast, icon, esc, preferences };
...
function showAuth(message = "") {
  ...
  window.DocSpaceVoice?.leave();
  document.querySelectorAll('dialog[open]').forEach(dialog => dialog.close());
  Object.assign(state, {view:"channels",channel:"général",peer:null,messages:[],users:[],conversations:[],reactions:{},unread:{},drafts:{},reply:null,attachment:null,friends:{friends:[],pending:[],requests:[]}});
  $("message-input").value = "";
...
function navigate(view, peer = null, channel = state.channel) {
  window.DocSpaceFeatures?.beforeNavigate();
  rememberDraft();
  stopTyping();
  window.closeDocSpaceGames?.();
  document.getElementById("godot-frame")?.remove();
  document.getElementById("page").classList.remove("game-page", "voice-page");
  state.view = view;
  state.peer = peer;
  ...
  window.dispatchEvent(new CustomEvent("docspace:navigate", {detail:{view,peer,channel}}));
  if (view === "arcade") {
...
function renderMembers() {
  const users = state.users.filter((u) => u.status !== "invisible" || keyName(u.username) === keyName(state.me?.username));
  $("members").innerHTML =
...
function renderProfile(p) {
  state.profiles[keyName(p.username)] = p;
  ...
  const color = /^#[a-f0-9]{6}$/i.test(p.profileColor || "")
    ...
    : "#65609c";
  $("members").innerHTML =
    `<div class="profile-card"><div class="profile-banner" style="background:${color}"></div>${avatarHTML(p.username, p.avatar)}<h2>${esc(p.username)}</h2><small>${p.status === "offline" ? "Hors ligne" : "En ligne"}</small><div class="profile-divider"></div><h3>À propos</h3><p>${esc(p.bio || "Ce profil attend encore quelques mots.")}</p><div class="profile-divider"></div><h3>Membre depuis</h3><small>${p.joinDate ? new Date(p.joinDate).toLocaleDateString("fr-FR", { month: "long", year: "numeric" }) : "Les débuts de DocSpace"}</small><div class="profile-divider"></div>${friendActionHTML(p.username)}</div>`;
}
function friendActionHTML(name) {
  const inList = list => (list || []).some(u => keyName(typeof u === 'string' ? u : u.username) === keyName(name));
  if(keyName(name) === keyName(state.me?.username)) return '';
  let button = inList(state.friends.friends) ? '<button class="friend-confirmed" disabled>✓ Déjà amis</button>' : inList(state.friends.pending) ? '<button disabled>Demande envoyée</button>' : inList(state.friends.requests) ? `<button class="primary" data-accept-friend="${esc(name)}">Accepter la demande</button>` : `<button class="primary" data-add-friend="${esc(name)}">Ajouter en ami</button>`;
  return `<div class="friend-action" data-friend-name="${esc(name)}">${button}</div>`;
}
function refreshFriendActions() { document.querySelectorAll('[data-friend-name]').forEach(el => {el.outerHTML = friendActionHTML(el.dataset.friendName);}); }
...
function attachmentHTML(a) {
  ...
  if (a.isVoiceClip || /\.(mp3|ogg|wav|m4a)$/i.test(url))
    media = `<audio controls preload="none" src="${esc(url)}"></audio>`;
  if (!a.isVoiceClip && /\.(mp4|webm)$/i.test(url))
...
function renderMessage(m, previous) {
  ...
  window.DocSpaceFeatures?.decorateMessage(el,m);
  return el;
}
...
function renderSettings(tab) {
  ...
  if (window.DocSpaceFeatures?.settings(tab,panel)) return;
  if (tab === "profile") {
  ...
  if (tab === "voice") {
    ...
    panel.insertAdjacentHTML("beforeend", `<label style="margin-top:18px">Caméra<select id="camera-device"><option value="">Caméra par défaut</option></select></label><p><small>La caméra démarre uniquement quand tu l’actives dans un salon vocal.</small></p>`);
...
socket.on("dm_history", (d) => {
  if (state.view === "dms" && keyName(d.username) === keyName(state.peer)) {
    ...
    state.messages = d.messages || [];
    renderMessages();
    if (state.messages.length && !document.hidden && document.hasFocus())
...
for (const event of ["dm_sent", "dm_received"])
  socket.on(event, (m) => {
    ...
    if (state.view === "dms" && keyName(state.peer) === keyName(peer)) {
      appendMessage(m);
      if (!document.hidden && document.hasFocus()) socket.emit("mark_dm_read", { username: peer, messageId: m.id });
...
document.addEventListener("submit", async (e) => {
  ...
  if (form.id === "youtube-form") {
    ...
    let id;
    try {
      const u = new URL(form.elements.video.value);
    ...
    window.DocSpaceSocial?.rememberVideo(id);
...
async function openGodot() {
  if(window.DocSpaceFeatures) return window.DocSpaceFeatures.openGodot("orbit-garden");
