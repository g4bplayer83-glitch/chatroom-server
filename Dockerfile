...
COPY godot/orbit-garden/ /project/
RUN mkdir /out && godot --headless --path /project --editor --import && godot --headless --path /project --export-release Web /out/index.html && test -s /out/index.wasm
...
RUN npm ci --omit=dev
COPY . .
COPY --from=godot-export /out/ /app/public/games/orbit-garden/
