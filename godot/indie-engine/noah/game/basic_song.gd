...
func _ready() -> void:
	...
	# Converted projects may need to show a native intro/cutscene after every
	# gameplay system is ready but before the countdown and audio start.
	if has_method(&"before_song_start"):
		await call(&"before_song_start")
...
func _on_conductor_new_step(current_step: int, measure_relative: int):
	# Offset shifts the modulo phase; it must not shrink the interval (or make
	# it zero/negative for valid Codename events).
	if bop_rate > 0 and (current_step - bop_rate_offset) % bop_rate == 0:
...
func note_miss(note: Note, lane: int, strum_manager: StrumManager):
	if note and note.note_type in ["hurt", "heal", "ghost", "instakill"] and not note.hit:
		return
