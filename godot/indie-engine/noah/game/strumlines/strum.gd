...
var NOTE_PRELOAD = preload("res://noah/game/note/note.tscn")
var SPLASH_PRELOAD = preload("res://noah/game/note/note_splash.tscn")
...
func stop_for_game_over() -> void:
	...
	pressing = false
	hold_cover_sprite.visible = false
	for active_note in note_list.duplicate():
		if is_instance_valid(active_note):
