...
func _process(delta) -> void:
	...
	if Input.is_action_just_pressed("menu_cancel"):
		if options == pages.difficulties:
			load_page("default")
			update(0)
		else:
			select_option(options.keys().find("resume"))
...
func select_option(i: int):
	...
	match option:
		...
		"back":
			...
			update(0)
		_:
			# Among Funk carries custom names such as V1 and V1.1. Treat any
			# option matching the current Song's difficulty dictionary as valid.
			if GameManager.current_song and GameManager.current_song.difficulties.has(option):
