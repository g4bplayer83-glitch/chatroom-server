...
func refresh_audios():
	...
	for strum in ChartManager.strum_data.size():
		...
		if track < vocal_tracks.size():
			playback.set_stream_volume(vocal_tracks[track], linear_to_db(ChartManager.strum_data[track]['volume']))
...
func remove_note(lane, time: float = -1):
	...
	if index < note_nodes.size() and index >= 0:
