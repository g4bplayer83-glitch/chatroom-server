			...
			if playback:
				for strum in ChartManager.strum_data.size():
					...
					if track < vocal_tracks.size():
						playback.set_stream_volume(vocal_tracks[track], linear_to_db(1))
	...
	if ChartManager.chart:
		...
		var snapped_position: Vector2i = Vector2i(
			grid.get_grid_position(
				mouse_position,
				grid.grid_size * Vector2(conductor.numerator * conductor.denominator / chart_snap, 1)
			)
		)
		
		## Song Start Offset Marker
		rect = Rect2(
			grid_offset - $"Grid Layer/Parallax2D".scroll_offset + Vector2(
				time_to_y_position(song_position - ChartManager.chart.offset + start_offset) - 2,
				grid.get_real_position(Vector2(0, 0)).y
			),
