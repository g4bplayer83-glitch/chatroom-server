...
const SHIT_RATING_WINDOW: float = 0.166
...
func get_current_song() -> Song:
	...
	if current_week_song >= 0 and current_week_song < week_songs.size():
		return week_songs[current_week_song]

	push_error("(GameManager) No current story song is available.")
	return current_song
